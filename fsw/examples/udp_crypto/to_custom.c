...
int32 TO_CustomInit(void)
{
    ...
    TO_EnableOutputCmd( (CFE_SB_Msg_t *) "\x18\x80\xC0\x00\x00\x13\x02\x00\x31\x32\x37\x2E\x30\x2E\x30\x2E\x31\x00\x00\x00\x00\x00\x00\x00\x93\x13");
...
{
    int32 iStatus = 0;
    uint8 ingest[1145]; /* Process Security - Variable */
    ...
    /* Apply Security - START */
    CFE_PSP_MemCpy(ingest, pMsg, size);
    iStatus = Crypto_TM_ApplySecurity((char*) ingest, (int*) &size);
    /* Apply Security - END */

    iStatus = IO_TransUdpSnd(&g_TO_CustomData.udp, (uint8 *) ingest, size);
