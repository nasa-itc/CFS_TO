...
typedef struct
{
    AOS_SDLP_FrameInfo_t frameInfo;
    AOS_SDLP_ChannelConfig_t vcConfig;
    uint8 ocfBuff[4];
...
typedef struct
{
    AOS_SDLP_GlobalConfig_t mcConfig;
    uint8                  mcFrameCnt;
    TO_CustomVChnl_t       vc;
} TO_CustomMChnl_t;
...
{
    TO_CustomMChnl_t    mc;
    uint8               buffer[TO_CUSTOM_TF_SIZE + AOS_SYNC_ASM_SIZE];
} TO_CustomPChnl_t;
...
//static const uint16    iCaduSize = TO_CUSTOM_TF_SIZE + AOS_SYNC_ASM_SIZE; 
...
int32 TO_CustomInit(void)
{
    ...
    AOS_SDLP_InitIdlePacket(pIdlePacket, &idlePattern[0],
    ...
    g_TO_CustomData.socket.pc.mc.mcConfig.hasFecf = TO_CUSTOM_TF_ERR_CTRL;
    g_TO_CustomData.socket.pc.mc.mcFrameCnt = 0;

    /* NOTE: We are setting VC ID to: Socket: 0 */

    /* Set channel config table */
    AOS_SDLP_ChannelConfig_t chnlConfig[TO_CUSTOM_NUM_CHNL] =
    {
        {6, 0, 0, 0, 0, 0, TO_CUSTOM_TF_OVERFLOW_SIZE},
    };

    pChnl = &g_TO_CustomData.socket.pc;
    CFE_PSP_MemCpy((void* )&pChnl->mc.vc.vcConfig, (void* )&chnlConfig[0],
                   sizeof(AOS_SDLP_ChannelConfig_t));

    if (AOS_SDLP_InitChannel(&pChnl->mc.vc.frameInfo,
                            &pChnl->buffer[AOS_SYNC_ASM_SIZE],
...
int32 TO_CustomFrameStart(uint16 usRouteId)
{
    ...
    AOS_SDLP_FrameInfo_t* pFrameInfo = NULL;
    ...
    iStatus = AOS_SDLP_StartFrame(pFrameInfo);
...
int32 TO_CustomFrameSend(uint16 usRouteId, int32 iInStatus)
{
    ...
    iStatus = AOS_SDLP_FrameHasData(pFrameInfo);
    if (iStatus == 1)
    {
#ifdef AOS_DEBUG
        ...
        iStatus = AOS_SDLP_AddIdlePacket(pFrameInfo, pIdlePacket);
    }
    else
    {
        goto end_of_function;
...
//         iStatus = AOS_SDLP_SetOidFrame(pFrameInfo, pIdlePacket);
    }
    ...
    /* Complete Frame */
    iStatus = AOS_SDLP_CompleteFrame(pFrameInfo, pOcf);
    if (iStatus != TO_SUCCESS)
    {
        goto end_of_function;
    }
    ...
    /* Perform SDLS */
    iStatus = Crypto_AOS_ApplySecurity((uint8_t*)pFrameInfo->frame, 1786);
    if (iStatus != TO_SUCCESS)
    {
        goto end_of_function;
    }
    ...
    iCaduSize = AOS_SYNC_Synchronize(pChnl->buffer, (char*) AOS_SYNC_ASM_STR, 
                                    (uint8_t) AOS_SYNC_ASM_SIZE,
...
int32 TO_CustomProcessPacket(CFE_MSG_Message_t *pMsg, uint16 usRouteId)
{
    ...
    /* Add Packet */
    iStatus = AOS_SDLP_AddPacket(pFrameInfo, pMsg);
    if (iStatus >= 0)
    {
        iStatus = TO_SUCCESS;
    }
